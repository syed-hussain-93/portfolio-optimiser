...
class Portfolio:
    ...
    def __init__(self, assets: List = []) -> None:
        ...
        self.portfolio_data = pd.concat([asset.data['Close'] for asset in self.assets], axis=1).dropna()
        self.portfolio_data.columns = [asset.name for asset in self.assets]

        self.assets_daily_returns = pd.concat([asset.daily_returns for asset in self.assets], axis=1).dropna()
    ...
    def optimise_with_risk_tolerance(self, risk_tolerance: float):
        ...
        result = minimize(
            lambda w: self._portfolio_std(w) - risk_tolerance*self._portfolio_return(w),
            self.set_random_weights(),
            constraints = [{'type': 'eq','fun': lambda w: np.sum(w) - 1.}], 
            bounds = [(0., 1.) for i in range(self.size)]
        )
    ...
    def optimise_sharpe_ratio(self):
        ...
        result = minimize(
            lambda w: -(self._portfolio_return(w) - RISK_FREE_RATE/100)/self._portfolio_std(w),
            self.set_random_weights(),
            constraints = [{'type': 'eq','fun': lambda w: np.sum(w) - 1.}], 
            bounds = [(0., 1.) for i in range(self.size)]
        )
    ...
    def optimise_with_expected_return(self, expected_return: float):
        ...
        result = minimize(
            ...
            constraints = [
                ...
                {'type': 'eq', 'fun': lambda w: self._portfolio_std(w) - expected_return}
                ], 
            bounds = [(0., 1.) for i in range(self.size)]
        )
        self.weights = result.x
